...
if ($jeha_profile == 'الإدارة المركزية للمعلومات' && $inserted_jeha !== 'الإدارة المركزية للمعلومات') {
  ...
  if (!empty($_GET['id']) && !empty($_GET['type']) && $_GET['type'] == 'k_pub') {
    ...
    $sql = "SELECT ketab_num,ketab_date FROM `reports_info` WHERE  id=$id ";

    $sql_select = mysqli_query($conn, $sql);
    $sql_row = mysqli_fetch_assoc($sql_select);
    $inserted_ketab_num = $sql_row['ketab_num'];
    ...
    if (mysqli_query($conn, $sql2)) {
      ...
      while ($uploads_row = mysqli_fetch_assoc($sql_uploads_result)) {
        ...
        if (mysqli_query($conn, $sql2)) {
          ...
          echo "Error sql_uploads public ketab new: " . "<br>" . mysqli_error($conn);
          exit;
        }
      ...
      $sql9 = "INSERT INTO `dewan` (`e_id`, `d_id`, `e_year`, `dewan_num`, `dewan_date`, `ketab_num`, `ketab_date`, `sendfrom`, `brief`, `following_date`, `result`, `sendto`, `sendto_date`, `added_by`, `add_date`, `jeha`, `sader`, `d_attach`, `d_attach_extension`, `note`, `details_type`, `resala_type`, `origin_sendfrom`, `dewan_num_reply`, `dewan_date_reply`, `dewan_num_related`, `dewan_date_related`, `isPrivate`, `isReport`, `id_num`)  SELECT 0, 0, `e_year`, $r_num_max, '$today', $r_num_max, '$today', '$jeha_profile', `r_title`, `r_following_date`, '',`sendto`, `send_date`, '$user', current_timestamp(), '$jeha_profile', 1, '', '', '', '$details_type_new', `resala_type`, `jeha`, 0,'0000-00-00', 0,'0000-00-00', isPrivate, `isReport`,`id_num` FROM `reports_info` WHERE  id=$id";
