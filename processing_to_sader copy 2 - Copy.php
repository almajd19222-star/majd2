                  ...
                  if ($admin == 1 || $admin == 7) {
                      ...
                      if (mysqli_num_rows($sql_feech_info_result) > 0) {
                          ...
                          while ($feech_info_row = mysqli_fetch_assoc($sql_feech_info_result)) {
                              ...
                              if (mysqli_query($conn, $update_feech_info)) {
                                  $update_feech_info_sader="UPDATE feech_info SET
                                  sader=1 WHERE jeha='$inserted_jeha' AND details_type='$details_type' AND edbara_date='$inserted_edbara_date' AND edbara_num=$inserted_edbara_num  ORDER BY ketab_num ASC";
                                  mysqli_query($conn, $update_feech_info_sader);
...
if(!empty($_GET['id']) && !empty($_GET['type']) && $_GET['type']=='k_pub' ) {
  ...
  $inserted_ketab_date = $sql_row['ketab_date'];
    $sql2 = "UPDATE `reports_info` SET
      ...
      $sql9 = "INSERT INTO `dewan` (".$dewan_cols.")  SELECT 0, 0, `e_year`, $r_num_max, '$today', $r_num_max, '$today', `sendfrom`, `r_title`, `r_following_date`, '',`sendto`, `send_date`, '$user', current_timestamp(), '$jeha_profile', 1, '', '', '', '$details_type_new', `resala_type`, `jeha`, 0,'0000-00-00', 0,'0000-00-00', isPrivate, `isReport` FROM `reports_info` WHERE  id=$id";
