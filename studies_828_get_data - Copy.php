...
$columns = array(
  ...
  array( 'db' => '`t1`.`id`',                     'dt' => 0, 'formatter' => function( $d, $row ) {
    if($row['3']=='صادر'){
      ...
      if($hasPerm_delete == '1'){
        return '<a href="delete.php?id='.$d.'&details_type='.$row['3'].'&type='.$row['5'].'&general_code='.$row['6'].'&delete_studies_828=true" onclick="'."return confirm('متأكد من الحذف؟');".'"><i style="font-size: 2rem" class="zwicon-delete"></i></a>';
      }else{
        return '';
      }
    }
  ...
  array( 'db' => '`t1`.`id`',                     'dt' => 1, 'formatter' => function( $d, $row ) {
    return '<a href="studies_828.php?id='.$d.'&edit=1&details_type='.$row['3'].'&type_code='.$row['5'].'&edit_process=0"><i style="font-size: 2rem" class="zwicon-edit-circle"></i></a>';},'field' => 'id' ),
  
  array( 'db' => 'id',                            'dt' => 2, 'formatter' => function( $d, $row ) {
    return '<a target="_blank" href="print-elements.php?edit=1&id='.$d.'&type='.$row['5'].'&jeha='.$row['4'].'&details_type='.$row['3'].'"><i style="font-size: 2rem" class="zwicon-printer"></i></a>';},'field' => 'id' ),
  ...
  array( 'db' => '`t1`.`place_name`',                  'dt' => 9, 'field' => 'place_name' ),
  array( 'db' => '`t1`.`name`',                   'dt' => 10, 'field' => 'name' ),
  array( 'db' => '`t1`.`fname`',                  'dt' => 11, 'field' => 'fname' ),
  array( 'db' => '`t1`.`lname`',                  'dt' => 12, 'field' => 'lname' ),	
  

  array( 'db' => '`t1`.`added_by`',               'dt' => 13, 'field' => 'added_by' ),
  array( 'db' => '`t1`.`add_date`',               'dt' => 14, 'field' => 'add_date' )
);
