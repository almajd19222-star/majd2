...
if(!empty($_POST['place_name'])){
    ...
    $sql="DELETE FROM coordinates WHERE `general_code`='$general_code'";
    if(mysqli_query($conn, $sql)){                         
    }else{
        echo "Error DELETE coordinates: " . "<br>" . mysqli_error($conn);
        exit;
    }
if(!empty($_POST['easting']) && !empty($_POST['northing'])) { 
     
    $zone = mysqli_real_escape_string($conn, $_POST['zone']);
    $x = mysqli_real_escape_string($conn, $_POST['easting']);
    $y = mysqli_real_escape_string($conn, $_POST['northing']);
    $longitude= $zone.' '.$x.' '.$y;
